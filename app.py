...
SCRIPT_URL = "https://script.google.com/macros/s/AKfycbysssfJn1xIkXkV_d5DB1EdrrZWsGn4WM2ZftX5OM62bCsgNzAsf5tYh_F8Tvubqqkx/exec"

# ลิงก์ Google Sheets หลักสำหรับดึงข้อมูลตั้งต้นมาแสดงผล
sheet_url = "https://docs.google.com/spreadsheets/d/1QoVGcHIA8Ui-iiUUaCYvwvuPDtnWmcl5TtOpwVCELUQ/edit?usp=sharing"
...
def save_to_sheets(student_list):
    # ส่งข้อมูลทั้งหมดไปเซฟลง Google Sheets ผ่าน Web App ท่อพิเศษนี้
    try:
        response = requests.post(SCRIPT_URL, data=json.dumps(student_list))
...
if st.button("💾 บันทึกข้อมูลลง Google Sheets (เซฟถาวร)", type="primary", use_container_width=True):
    with st.spinner("กำลังส่งข้อมูลอัปเดตลง Google Sheets..."):
        if save_data_via_web_app(st.session_state.students):
            st.success("✅ บันทึกสถิติข้อมูลลง Google Sheets สำเร็จแล้วครับ!")
        else:
